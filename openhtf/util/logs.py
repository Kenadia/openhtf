...
We define two logging trees: `openhtf` and `openhtf.output.servers`. Within each
tree, we attach handlers to the top-level logger only. All descendants within a
tree have their logs propagate upward to the top-level.
...
Within the `openhtf` tree we distinguish between framework logs and test record
logs. Whereas framework logs are saved to the records of all running tests, test
record logs are only saved to the record of the test with a matching test_uid.
Server logs are not saved to the test record at all.
...
See the console_output module for tools for printing to the CLI.

During development you may want to increase the verbosity level to view logs and
full tracebacks of errors. The verbosity flag can be used as follows:
  - Default: logs are not printed.
  - `-v`: Framework and test record logs are printed at the INFO level and up.
  - `-vv`: Framework and test record logs are printed at DEBUG and up.
  - `-vvv`: Framework, test record, and server logs are printed at DEBUG and up.

In addition, the --quiet flag and CLI_QUIET parameter from the console_output
module will override the verbosity setting and suppress all CLI output.

------------------------------- Test record logs -------------------------------

Test record logs are typically generated by test authors. Often this is
done with the logger attached to the TestApi passed into test phases, e.g.:
...
def initialize_logging():
  ...
  # Define top-level logger for OpenHTF logs including framework logs and logs
  # specific to individual test runs.
  htf_logger = logging.getLogger(LOGGER_PREFIX)
  htf_logger.propagate = False
  htf_logger.setLevel(logging.DEBUG)

  # Define top-level logger for server logs. These logs are not relevant to most
  # users and generally don't belong in the test record.
  server_logger = htf_logger.getChild('output.servers')
  ...
  # According to the verbosity arg, configure a handler to print to the CLI.
  ...
  # Only print server logs if -vvv or greater verbosity is specified.
  if CLI_LOGGING_VERBOSITY >= 3:
    server_logger.addHandler(cli_handler)
  else:
    server_logger.addHandler(logging.NullHandler())
