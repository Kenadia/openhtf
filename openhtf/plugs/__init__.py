...
class BasePlug(object):
  ...
  def get_plug_name(cls):
    """Returns the plug's name, e.g. `openhtf.plugs.user_input.UserInput`.

    The name of a plug which extends another plug is a comma-separated list of
    class names. This allows frontends to make use of base class information to
    render subclassed plugs appropriately.
    """
    ignored_classes = {FrontendAwareBasePlug, BasePlug, object}
    mro = [
